...
contract Taxpayer {
  ...
  modifier onlyValidAddress(address _addr) {
    require(isValidSpouseAddress(_addr), "Invalid or restricted spouse address");
    _;
  }
  ...
  function marry(address newSpouse) public onlyValidAddress(newSpouse) {
    ...
    require(!sp.isMarriedState() || sp.spouseAddress() == address(this), "Partner unavailable");
  ...
  function setWonLottery() external {
    require(authorizedLotteries[msg.sender], "Only an authorized lottery can notify win");
    lotteryWins++;
    setExtendedTaxAllowance();
  }
  ...
  function isValidSpouseAddress(address _addr) public view returns (bool) {
    if (_addr == address(0) || _addr == BURN_ADDRESS || _addr == address(this) || uint160(_addr) <= 9 || _addr.code.length == 0) {
      return false;
    }
